    ...
    if (userRepo.trim()) {
      try {
        const response = await fetch('./api/user', {
          ...
          body: JSON.stringify({ userRepo: userRepo.trim() }),
        });
        ...
        if (response.ok) {
          ...
          if (data.exists) {
            ...
            router.push(`/${username}?repo=${repoName}`);
          } else {
            toast.error('Repo does not exist on GitHub');
          }
        } else {
          toast.error('Error verifying username or repo. Please try again.');
        }
      } catch (error) {
        toast.error('An unexpected error occurred. Please try again.');
      }
    } else {
      toast.error('Please enter a username and repo.');
    }
  };
    ...
    <>
      ...
      <div className="flex flex-col justify-between min-h-screen">
        ...
        <header className="p-4 flex flex-row items-center">
          ...
          <h1 className="text-xl font-bold text-white mt-6">Heatmap Generator</h1>
        </header>

        {/* Main Content */}
        <main className="flex flex-col justify-center items-center flex-grow px-8 sm:px-4">
          <h1 className="text-4xl font-light text-transparent bg-clip-text bg-gradient-to-r from-green-200 to-green-400 tracking-widest mb-8">
            Let's, Begin...
          </h1>
          <form className="flex items-center w-full sm:w-2/3 lg:w-1/3 h-1/3" onSubmit={handleSubmit}>
            <div className="flex items-center bg-white rounded-full shadow-md p-2 w-full">
              ...
              <button
                type="submit"
                className="bg-green-500 text-white p-3 rounded-full hover:bg-green-600  focus:outline-none focus:ring-4 focus:ring-green-500 focus:ring-opacity-50 transition duration-200 flex items-center justify-center"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-5 w-5"
        ...
        <footer className="p-4 text-center text-gray-600">
          Made with ❤️ | <a href="https://github.com/vansh-codes/Heatmap-Generator" target="_blank" rel="noreferrer noopener">Heatmap Generator</a>
        </footer>
      </div>
    </>
  );
